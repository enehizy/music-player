...
function TrackTable({tracks,albumImage}) {
  ...
  return (
    <table className="w-[95%]  border-separate border-spacing-4  pl-6 transition-all -mt-6 " >
        ...
        }}  key={index} className={`${track.uri === trackId&&'bg-[#d9d5e9] shadow-[#8568f5]  '} bg-transparent transition-all duration-75 shadow-md hover:shadow-[#8568f5] font-semibold text-gray-500 group`}>
            <td className=' font-black block group-hover:hidden p-2'>{index +1} </td>
            <td className='hidden group-hover:block pt-3'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#8568f5" className="size-6">
