          ...
          <div className="grid md:grid-cols-3 justify-center items-center md:px-24 gap-10 2xl:grid-cols-4">
            {popularArtists.data.map((artist) => (
              <Link
                key={artist.id}
                to={`${artist.id}`}
                className="w-[250px] aspect-auto flex flex-col justify-center items-center gap-3"
              >
                <img
                  className="w-full h-full"
                  src={
                    artist.images?.[0]?.url ?? "https://placehold.co/250x250"
                  }
                />
                ...
                <p className="text-center text-base font-semibold text-gray-500 flex gap-2">
                  ...
                  {numeral(artist.followers.total).format("0.0a")}
                </p>
              </Link>
            ))}
          </div>
