...
function NavLink({ label }) {
  ...
  return (
    <li className="grow">
      <Link
        to={getLinkFromLabel}
        className={`flex px-16 py-1 grow font-bold  gap-2  justify-center items-center capitalize ${location.pathname == getLinkFromLabel || getRegexForLink.test(location.pathname) ? "bg-white rounded-xl  text-white bg-gradient-to-r from-[#3dc3c0] to-[#8568f5] " : ""}`}
      >
        {label == "home" && (
          <Home isActive={location.pathname == getLinkFromLabel} />
        )}
        {label == "artists" && (
          <Artist
            isActive={
              location.pathname == getLinkFromLabel ||
              getRegexForLink.test(location.pathname)
            }
          />
        )}
        ...
        {label == "trending" && (
          <Trending isActive={location.pathname == getLinkFromLabel} />
        )}
        {label}
      </Link>
    </li>
